...
from tools.okx_auto_health import evaluate_health, list_auto_trading_pids, load_state
...
def test_list_auto_trading_pids_ignores_tmux_wrapper(monkeypatch) -> None:
    output = "\n".join(
        [
            "33268 tmux new-session -d -s okx_auto cd /repo && python run_auto_trading.py",
            "33269 /repo/.venv/bin/python run_auto_trading.py",
        ]
    )

    monkeypatch.setattr(
        "tools.okx_auto_health.subprocess.check_output",
        lambda *_args, **_kwargs: output,
    )

    assert list_auto_trading_pids() == [33269]


def test_evaluate_health_allows_live_chan_with_empty_latest_signal_state(tmp_path: Path) -> None:
    state_path = tmp_path / "state.json"
    state_path.write_text(
        json.dumps(
            {
